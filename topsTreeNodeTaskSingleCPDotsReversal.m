classdef topsTreeNodeTaskSingleCPDotsReversal < topsTreeNodeTask
   ...
   methods
      ...
      % following:
      % 1/ sets the appropriate 'timeout' field of the state machine's
      % state, depending on:
      %     a/ viewing duration of current trial
      %     b/ whether there is CP or not
      % 2/ sets the appropriate 'next' field of the state machine's state
      function nextState = dotsFirstEpoch(self)
   ...
   methods (Access = protected)
      ...
      function prepareStateMachine(self)
          ...
          if trial.timeCP >= trial.viewingDuration
              self.isCP = false;
              self.setTrialData(self.trialCount,'presenceCP',false);
              self.timing.dotsDuration1 = trial.viewingDuration;
              self.timing.dotsDuration2 = 0;
          else
              if rand < self.timing.probCP
                  self.isCP = true;
                  self.setTrialData(self.trialCount,'presenceCP',true);
      ...
      function initializeStateMachine(self)
         ...
         % state
         pdbr    = {@setNextState, self, 'isCP', 'showDotsEpoch1', 'showDotsEpoch2', 'waitForChoiceFX'};
         ...
         states = {...
            ...
            'preDots'           {}       {}       0                         {}      ''                ; ...
            'showDotsEpoch1'    showdFX  {}       self.timing.dotsDuration1 hided   ''                ; ...
            'showDotsEpoch2'    showdFX  {}       self.timing.dotsDuration2 hided   'waitForChoiceFX' ; ...
